...
const ScienceBoard: React.FC<ScienceBoardProps> = ({ level, powerUps, onCorrect, onWrong, usePowerUp }) => {
  ...
  return (
    <div className={`w-full h-full flex flex-col md:flex-row p-2 md:p-6 bg-slate-950 relative overflow-hidden font-quicksand touch-lock`}>
          ...
          <div className="grid grid-cols-4 md:grid-cols-2 gap-2">
            {level.scientificData?.hiddenItems.map((item: any) => (
              <div key={item.id} className={`aspect-square rounded-xl flex flex-col items-center justify-center border-2 transition-all duration-500 ${foundItems.includes(item.id) ? 'bg-emerald-500 border-white scale-100 shadow-xl' : 'bg-white/5 border-white/5 opacity-20 scale-90'}`}>
                <i className={`fas ${item.icon} text-base md:text-2xl text-white`}></i>
              </div>
            ))}
          </div>
        </div>
      ...
      <div 
        ...
        className="flex-grow relative cursor-none select-none rounded-[2rem] md:rounded-[4rem] border-2 md:border-8 border-white/5 shadow-2xl overflow-hidden bg-emerald-950/20 group"
      >
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle, #34d399 1px, transparent 1px)', backgroundSize: '40px 40px' }}></div>

        {level.scientificData?.hiddenItems.map((item: any) => {
          const isFound = foundItems.includes(item.id);
          const isLensRevealed = revealedByLens.includes(item.id);
          const isVisible = isFound || isLensRevealed || (isScanning && Math.sqrt(Math.pow(scanPos.x - item.x, 2) + Math.pow(scanPos.y - item.y, 2)) < 18);

          return (
            <div 
              key={item.id}
              className={`absolute transition-all duration-700 transform -translate-x-1/2 -translate-y-1/2 ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-50 pointer-events-none'}`}
            ...
            >
               <div className={`relative p-4 md:p-10 rounded-full border-2 md:border-4 shadow-2xl ${isFound ? 'bg-emerald-500 border-white' : 'bg-white/10 border-white/10'}`}>
                  <i className={`fas ${item.icon} text-xl md:text-6xl text-white`}></i>
               ...
               {isFound && <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 bg-white text-emerald-800 px-2 py-0.5 rounded-full font-black text-[8px] md:text-xs whitespace-nowrap border border-emerald-100 uppercase">{item.label}</div>}
            </div>
          );
        })}
      ...
      {discoveryDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-3xl animate-fade-in">
           <div className="bg-white rounded-[2.5rem] md:rounded-[4rem] shadow-2xl border-[8px] md:border-[16px] border-emerald-500 p-6 md:p-12 max-w-xl w-full text-center animate-pop-in">
              <div className="w-20 h-20 md:w-32 md:h-32 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-8 border-4 border-emerald-200">
                 <i className={`fas ${discoveryDetail.icon} text-3xl md:text-6xl text-emerald-600`}></i>
              </div>
              <h2 className="text-2xl md:text-5xl font-fredoka text-gray-800 mb-2 md:mb-4 uppercase">{discoveryDetail.label}</h2>
              <div className="bg-emerald-50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-emerald-100 text-left mb-6 md:mb-10">
                <p className="text-gray-700 text-xs md:text-xl font-medium leading-relaxed italic">"{discoveryDetail.desc}"</p>
              </div>
      ...
      <button onClick={() => handlePowerUp('darwinLens')} className="absolute bottom-4 right-4 md:bottom-10 md:right-10 w-12 h-12 md:w-20 md:h-20 bg-yellow-400 rounded-2xl md:rounded-[2.5rem] shadow-2xl border-4 border-white flex items-center justify-center text-white active:scale-90 transition-all z-40">
         ...
         <span className="absolute -top-1 -right-1 bg-emerald-800 text-white text-[10px] md:text-sm font-black px-2 py-0.5 rounded-full border-2 border-white">{powerUps.darwinLens}</span>
      </button>
    </div>
  );
};
